...
def favorite_activities():
    ...
    fun_activity = []
    for day in days:
        activity = input(f'what is your fav activity on {day}')
        fun_activity.append(activity)
    ...
    for i in range(len(fun_activity)):
        print(f'On {days[i]}s, your favorite activity is to {activity}.')
...
def temp_by_day():
    ...
    for day in days:
        message = " "
        temp = int(input(f"today is {day} It is how many degrees outside?"))
        if(temp < 50):
            message = 'Brr, put on a jacket!'
        elif(temp >= 50 and temp <= 65):
            message = "Cozy, grab a sweater"
        elif(temp > 65):
            message = "Put on some sunscreen!"        
        print(message)
...
def temp_by_day_continuous():
    ...
    # Spring has sprung!
    temp2 = int(input("What is the temp outside"))
    while temp2 < 65:
